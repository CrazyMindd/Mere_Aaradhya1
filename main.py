...
bot = Client("bot",
             bot_token= "6407728976:AAGwxwCRfn3pwCJw0nIW3HOdQwyxC5RwW4k",
             api_id= 28773160,
             api_hash= "37bdcd1fce6d5edc6f5f8b28b44c9e8c"
)
...
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:
                cc = f'[ 🎬 ] **Vid ID : **{str(count).zfill(3)}\n**Video Title :** {name1}({res}) 『 🅲🆁🅰🆉🆈_🅼🅸🅽🅳 』.mp4\n**Batch Name :** {b_name}\n\n**Extracted By ➤** {CR}'
                cc1 = f'[ 📕 ] **Pdf ID : **{str(count).zfill(3)}\n**File Title :** {name1} 『 🅲🆁🅰🆉🆈_🅼🅸🅽🅳 』.pdf\n**Batch Name :**{b_name}\n\n**Extracted By ➤** {CR}'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                        await copy.copy(chat_id = -1002010646599)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
